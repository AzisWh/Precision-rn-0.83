...
export const getDeliveryPicList = async (
  ...
  if (error) {
    console.log('❌ getDeliveryPicList error:', error.message);
...
export const getDeliveryPicCounts = async (): Promise<
...
> => {
  ...
  if (error) {
    console.log('❌ getDeliveryPicCounts error:', error.message);
    throw { status: 500, message: error.message } as ApiError;
  }
